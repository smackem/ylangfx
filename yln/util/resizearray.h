...
#define array_get(array_ptr, type, index) \
    (*(type *) array_get_ptr(array_ptr, sizeof(type), index))
#define array_set(array_ptr, type, index, item) \
    (*(type *) array_get_ptr(array_ptr, sizeof(type), index) = item)

void array_init_impl(ResizeArray *array, size_t sizeof_item, int initial_capacity);
#define array_init(array_ptr, type, initial_capacity) \
    (array_init_impl(array_ptr, sizeof(type), initial_capacity))

void array_free(ResizeArray *array);

void *array_get_ptr_grow(ResizeArray *array, size_t sizeof_item, int index);
#define array_push(array_ptr, type, item) \
    (*(type *) array_get_ptr_grow(array_ptr, sizeof(type), (array_ptr)->size) = item)
...
#define array_pop(array_ptr, type) \
    (*(type *) array_pop_impl(array_ptr, sizeof(type)))

void array_push_range(ResizeArray *array, size_t sizeof_item, void *source, int source_index, int source_length);
